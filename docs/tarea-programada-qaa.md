...
- cta_agenda_url y cta_agenda_label: salen de .textos.* (página de citas de Google Calendar de Alejandro). DEFENSIVO: si el valor trae un código `<iframe ...>` completo (Alejandro pega el snippet de Google tal cual), extrae el src= y usa esa URL como link. Si viene vacío o el curl falla: usa "[PEGAR LINK DE AGENDA]" en los borradores y repórtalo como PENDIENTE.
...
- Cotiza sobre **m² habitables + Circulaciones y grosor de muros** (regla 2026-06-11, sustituye a la vieja "no aplicar circulación"): calcula el subtotal de m² habitables y SÚMALE el porcentaje `circulacion` del catálogo del PASO 0 (campo en `cotizacion_2026`/`app` del aurum-catalogo.json; default 0.12 = +12%, **pendiente % final de Mariana**, rango 0.10–0.15). Esa circulación: (1) aparece como una LÍNEA VISIBLE del programa de áreas — "Circulaciones y grosor de muros (+12%)" — tanto en el brief como en el bloque del estimado del correo, y (2) ENTRA a la base de los 3 servicios: cada uno (Llave en mano / Ejecutivo / Diseño) se calcula sobre (m² habitables + circulación) × precio × multiplicador_lujo. Los no-habitables siguen sin cotizar. Triple assert obligatorio (incluye que la base de cotización = habitables + circulación, no solo habitables). Misma regla para leads del QAA viejo y de la web — ambos usan este mismo cálculo.
...
- Renderiza brief-template.html (quita comentarios, reemplaza {{...}}, genera filas de TABLA_HAB_ROWS y TABLA_NOHAB_ROWS). Placeholders de circulación (regla 2026-06-11): `{{CIRC_PCT}}` = el % de circulación en texto (p.ej. "12%"), `{{CIRC_M2}}` = m² de circulación = round(subtotal habitables × circulacion), `{{BASE_COTIZACION_M2}}` = subtotal habitables + circulación (= `{{SUBTOTAL_HAB}}` + `{{CIRC_M2}}`). Los 3 servicios de la sección 09 se calculan sobre `{{BASE_COTIZACION_M2}}`, NO sobre `{{SUBTOTAL_HAB}}`. Verifica que NO queden placeholders huérfanos {{...}}; si quedan o un assert falla, NO crees ese borrador y anótalo como error.
...
IMPORTANTE (rediseño 2026-06): la web YA NO muestra al cliente ni el rango de inversión ni el desglose de espacios — le PROMETE que "tu estimado personalizado te llega a tu correo en menos de 24 horas, revisado por un arquitecto". ESTE CORREO ES ESE ENTREGABLE. Es la primera vez que el cliente ve sus números: trátalo como la pieza estrella, no como un resumen.
El lead web trae el cálculo de la app (M2 habitables/totales, Rango bajo/alto MXN — calculados pero NO mostrados al cliente) y todas sus elecciones. La columna JSON conserva el payload completo, incluido `integrantes`: lista de los habitantes con etapa de edad (adulto/adolescente/nino) y si cada uno tiene recámara propia o comparte — ÚSALO en el brief (composición del hogar y justificación del nº de recámaras).
a) **Brief**: genera el brief IGUAL que en el PASO 5 usando los datos del renglón — terreno (Terreno m2; vacío→500), personas, nivel (Nivel; vacío→Casual), vehículos (Autos), integrantes del JSON, y AGREGA al programa los espacios de la columna Extras (vienen con nombre display, p.ej. "Terraza, Estudio / Oficina" — mapéalos al catálogo). RECALCULA m² y cotización con el catálogo del PASO 0 (fuente de verdad de hoy); si tu resultado difiere <10% del "M2 habitables" del renglón es normal (catálogo pudo cambiar); si difiere >10%, usa el tuyo y anótalo en el aviso. Usa el FOLIO que ya trae el renglón — no generes uno nuevo. Lo que el cuestionario web no captura (orientación, colonia, acabados…): "[POR DEFINIR]".
b) **COVER del correo** (3 bloques, ANTES del brief, mismo htmlBody):
   1. *Personalización*: saluda por su primer nombre y refleja SUS elecciones — estilo ("Nos encanta que te identifiques con el estilo {Estilo}…"), el carácter que eligió (campo caracter_display del JSON: Serena/Sobria/Cálida/De autor), sensaciones y momentos, los espacios extra que soñó, y su hogar ("pensada para ustedes {N}, con recámara para cada quien…"). Que se note que SÍ leímos su cuestionario. Tono Aurum: elegante, sobrio, segunda persona, español de México, sin exagerar.
   2. *EL ESTIMADO PROMETIDO* (la revelación): "como te prometimos, aquí está tu estimado preparado y revisado por nosotros" — ≈m² habitables, RANGO DE INVERSIÓN (formato $X.X – $X.X MDP, primera vez que lo ve), nº de recámaras, y la nota de que el brief abajo trae el programa de áreas completo espacio por espacio. Presentarlo con peso: es el momento de valor del correo.
   3. *Sesión de Diseño (CTA)*: si su renglón ya tiene "Sesión agendada", FELICÍTALO y confirma fecha/hora en vez de pedirle agendar. Si NO: invítalo a agendar — 45 min en videollamada con un arquitecto Aurum, sin costo y sin compromiso — con BOTÓN/LINK al cta_agenda_url del PASO 0; explica que al elegir horario recibirá la invitación de Google Calendar con el enlace de la videollamada (Meet) y recordatorios automáticos. Cierra con la nota de agenda limitada.
c) Subject del borrador: "{Folio} · Tu estimado Aurum — {Proyecto o primer nombre}". to=Email del renglón; cc=comercial@yodesarrollo.mx. NUNCA enviar.
