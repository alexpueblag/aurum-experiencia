...
LINK DE AGENDA (para el CTA del correo): `curl -sL "WEBHOOK?recurso=textos"` → JSON con .textos.cta_agenda_url (la página de citas de Google Calendar de Alejandro) y .textos.cta_agenda_label. Si cta_agenda_url viene vacío o el curl falla: usa el placeholder "[PEGAR LINK DE AGENDA]" en los borradores y repórtalo como PENDIENTE en el aviso final (significa que Alejandro aún no pega su link en la pestaña TEXTOS WEB del CRM).
...
a) **Detectar agendados**: lista los eventos de los próximos 14 días del calendario de Alejandro cuyo título/origen corresponda a la página de citas ("Sesión de Diseño" o el nombre que use la cita). Por cada evento con un invitado cuyo email exista en "LEADS - WEB" y cuyo renglón NO tenga ya "Sesión agendada": márcalo vía webhook → estado=SESIÓN AGENDADA, sesion="YYYY-MM-DD HH:MM", nota="Sesión {fecha} — {link del evento}".
